...
| `PickerField` + `PickerSheet` | Choosing an **entity** (a contact, a project) or any list > 8 items. Field looks like an input with a chevron; opens a stacked sheet with check marks and (when > 8, or always with `onCreate`) a search box. With `onCreate`, whatever she types is offered as "Crear «…»": the row is created right there with only its name (no full form), selected, and the sheet closes. | `PickerField`: `title`, `options: {value,label}[]`, `value`, `onChange`, `placeholder?` ("Ninguno"), `onCreate?: (typed) => Promise<string \| null>` (resolve the new id; null = the store refused and already toasted), `createLabel?` ("Nuevo contacto"). `PickerSheet` is used by it; you rarely need it directly. |
| `hooks/useQuickCreate` | The `onCreate` for every entity picker: `contact(name, relationship)`, `project(title, { contactId?, courseId?, price? })`, `expo(title, date)`, `course(name)`. Each stamps id + date, writes through the same optimistic store as the full sheet, toasts "Contacto creado · Ana" and returns the id. Row shapes come from the pure builders in `utils/quickCreate.ts` (tested), which produce exactly what the entity's own sheet would have saved with only the name filled. **Every picker that references a contact / piece / expo / course passes one of these** — a name she hasn't saved yet is never a dead end. | `const quick = useQuickCreate(); <PickerField … onCreate={(n) => quick.contact(n, "client")} createLabel="Nuevo contacto" />`. Render the picker even when there are zero options. |
| `SwipeRow` | Mail-style delete on any list row: drag left reveals a red "Eliminar", tapping it (or a full-length swipe) arms an inline "¿Eliminar «…»? No · Sí, eliminar" in the row's own footprint — the confirm step is never skipped. On pointer devices a trash button shows on hover / focus. Direction-locked after 8px so vertical drags still scroll (and PullToRefresh still works); one row open at a time; taps elsewhere close it. The sheet's Eliminar stays as the second path. | `label` (the title the question names), `question?` (override when the delete cascades — say what else goes), `onDelete: () => Promise<boolean>` (the AppContext `remove*`; the parent unmounts the row on true, the row closes itself on false), `disabled?`, `children` (the `.row-item` button). CSS in `components.css` (`.swipe-row*`). |
...
| `BottomTabs` | The floating glass tab pill, rendering `settings.tabs` in her order (default Dinero · Hoy · Agenda, 2–5 of anything in `data/nav.ts`). A new destination is added to the catalog there (+ `Route` in `hooks/useNavigation.ts`, + a case in `App.tsx::Screen`, + a skeleton layout) and is then offered in the editor and the drawer automatically. | `tabs`, `route`, `navigate`. |
| `TabBarEditor` | Ajustes → "Tu barra de abajo": the pinned modules as rows with left / right / remove, the rest as chips to add, a reset to the default. Saves on every tap; the live pill is the preview. | `value` (`TabRoute[]`), `onChange`. |
| `data/nav.ts` | The one destination catalog (route, label, icon, group, practice gate) plus the pure bar helpers: `normalizeTabs`, `isTabRoute`, `landingRoute`, `parentRoute`. `BottomTabs`, `Drawer` and `TabBarEditor` all read it, so the pill and the menu partition the same list. | — |
...
| `lib/device.ts` | `prefersAutoFocus()` — false on touch devices. **Every `autoFocus` and every programmatic focus on open goes through it**: on a phone the keyboard never appears until she taps a field herself. Search boxes (global search, find-in-note) are the deliberate exception — typing is their only purpose. | |
...
| `QuickAddFab` + `useFab` | **The one + button for the whole app**, mounted once in `App.tsx` (hidden on Ajustes and while a sheet is open). A tap rotates it and fans out: the current screen's primary action nearest her thumb, then `settings.quickActions`. It owns the generic create sheets (sale / expense / event / piece / contact / tarea / nota rápida), so "new anything" is two taps from anywhere. A screen registers its primary with `useFab({ key, label, icon, onPick }, hidden?)` from `context/FabContext` — `key` dedupes against a quick action of the same kind, `onPick` may close over screen state (Agenda's selected day). **Never render a `.fab` inside a screen.** | `QuickAddFab { primary, hidden }` (App only); `useFab(primary \| null, hidden = false)`. |
...
| `BugButton` | The little bug in the topbar, right after the menu / back chevron (alone in the ≥1024px rail) on every signed-in screen. One tap opens `FeedbackSheet`. Rendered by `App.tsx` as part of `topbarLeft`; same 44px pill as the magnifier. | `onClick`. |
| `TrendChart` | "Cómo vienes" — the last six months as one signed bar each, cash basis. Lives in Dinero → Balance (moved off Hoy at the pilot's request: money must not be the first thing she sees). | `payments`, `expenses`, `today`. |
| `lib/breadcrumbs.ts` | In-memory trail of the last 40 things that happened — routes visited (`useNavigation`), uncaught errors, rejected promises, `console.error` (messages only, objects become `[object]`). `installBreadcrumbs()` runs in `main.tsx`; `readBreadcrumbs()` is attached to every feedback report so the reader can retrace her steps. Never stored, never sent on its own. | |
| `FeedbackSheet` | "Cuéntale a Diego" — a falla / idea / pregunta with a message, sent through `api/feedback.ts` (row in `feedback` + mail to the admin via Resend). Opened from anywhere signed in via `useSession().openFeedback()`: the drawer's Angus group, Ajustes → Ayuda, and the crash screen. What travels with it: route, app version **and build sha** (`__BUILD_SHA__`, from `VERCEL_GIT_COMMIT_SHA`), viewport, language, timezone, connection type, online/standalone flags, the last five local diagnostics, the breadcrumb trail, and per-table loaded/total/truncated counts — messages and counts, never rows. | `route?`, `onClose`. Mounted once in `App.tsx` inside `AppProvider`. |
