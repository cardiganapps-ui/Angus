...
| `BugButton` | The little bug, bottom-left of every signed-in screen (opposite the +). One tap opens `FeedbackSheet`. Mounted once in `App.tsx`; hidden with the FAB while a sheet is open. | `onClick`. |
