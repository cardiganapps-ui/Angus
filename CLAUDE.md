...
- **Data flow — one context.** `AppContext` composes twelve `useCloudStore(workspaceId, …)` instances (projects, contacts, events, sales, payments, installments, expenses, recurring rules, event series, class groups, enrollments, attendance) and exposes typed `items` + `add/addMany/update/remove` per entity, plus `loading`, `error`, `refreshAll`, and the workspace `settings` + `updateSettings`. Screens consume via `useApp()`. Row ↔ entity mapping (snake_case ↔ camelCase) lives only in `data/rows.ts`.
...
- `styles/` — split by concern (`fonts`, `base`, `components`, `money`, `dashboard`, `drawer`, `settings`, `onboarding`, `charts`, `calendar`, `account`, `auth`, `responsive`, `dark`), aggregated by `index.css`. Keep files narrow.
...
- **Project** ("Obra") — a piece, commission, or series: `status` (idea / in_progress / on_hold / completed), `availability` (available / reserved / sold / not_for_sale / gifted), medium, dimensions, year, edition, location, start/due dates, price, `cost` (materials estimate), optional linked contact.
- **Contact** — `relationship` (lead / client / gallery / supplier / collaborator / other / student via enrollments). Leads carry a `leadStage` (new / contacted / negotiating / won / lost) and a `followUpDate`; Hoy's attention list surfaces follow-ups due today or overdue.
